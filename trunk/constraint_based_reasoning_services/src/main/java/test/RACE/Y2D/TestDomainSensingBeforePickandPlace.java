...
public class TestDomainSensingBeforePickandPlace {
	...
	private static void insertCurrentStateCurrentGoal(SpatialFluentSolver grounSpatialFluentSolver) {
		...
		setFluentintoNetwork(cons, grounSpatialFluentSolver, "atLocation", "fork1", "at_fork1_table1()", markings.JUSTIFIED, 23);
		setFluentintoNetwork(cons, grounSpatialFluentSolver, "atLocation", "knife1", "at_knife1_table1()", markings.JUSTIFIED,  23);
		...
		one.setSymbolicDomain("at_counter()");
		...
		two.setSymbolicDomain("at_cup1_counter()");
	...
	private static HashMap<String, Rectangle> getAssertionalRule(Vector<SpatialAssertionalRelation2> saRelations){
		...
		insertAtConstraint(recs, saRelations, "fork", 24, 28, 11, 23, true);
		insertAtConstraint(recs, saRelations, "knife", 34, 38, 13, 25, true);
		insertAtConstraint(recs, saRelations, "cup", 0, 0, 0, 0, true);
