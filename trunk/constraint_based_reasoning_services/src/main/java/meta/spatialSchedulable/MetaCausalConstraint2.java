...
public class MetaCausalConstraint2 extends MetaConstraint {
	...
	private HashMap<String,SimpleReusableResource3> resourcesMap;
	private HashMap<SimpleReusableResource3,HashMap<Activity,Integer>> currentResourceUtilizers;
	...
	public MetaCausalConstraint2(int[] capacities, String[] resourceNames, String domainName) {
		super(null, null);
		this.name = domainName;
		currentResourceUtilizers = new HashMap<SimpleReusableResource3,HashMap<Activity,Integer>>();
		resourcesMap = new HashMap<String, SimpleReusableResource3>();
		...
		for (int i = 0; i < capacities.length; i++) {
			...
			resourcesMap.put(resourceNames[i], new SimpleReusableResource3(varOH, valOH, capacities[i], this, resourceNames[i]));
		}
		
		// for every RRS just created, put it coupled with a vector of variables
		for (SimpleReusableResource3 rr : resourcesMap.values()) currentResourceUtilizers.put(rr,new HashMap<Activity, Integer>());
	}
...
//		(SpatialFluentSolver2)this.metaCS.getConstraintSolvers()[0]).getConstraintSolvers()[0]
		Vector<ConstraintNetwork> ret = new Vector<ConstraintNetwork>();
		for (int i = 0; i < ((ActivityNetworkSolver)(((SpatialFluentSolver2)this.metaCS.getConstraintSolvers()[0]).
				getConstraintSolvers()[1])).getVariables().length; i++) {
			if(((ActivityNetworkSolver)(((SpatialFluentSolver2)this.metaCS.getConstraintSolvers()[0]).getConstraintSolvers()[1])).
					getVariables()[i].getMarking().equals(markings.UNJUSTIFIED)){
//				System.out.println("inside: " +((ActivityNetworkSolver)(((SpatialFluentSolver2)this.metaCS.getConstraintSolvers()[0]).
//						getConstraintSolvers()[1])).getVariables()[i]);
				ConstraintNetwork nw = new ConstraintNetwork(null);
				nw.addVariable(((ActivityNetworkSolver)(((SpatialFluentSolver2)this.metaCS.getConstraintSolvers()[0]).
						getConstraintSolvers()[1])).getVariables()[i]);
				ret.add(nw);
			}
		}
	...
	private ConstraintNetwork expandOperator(SimpleOperator possibleOperator, Activity problematicActivity) {		
		...
		ActivityNetworkSolver groundSolver = (ActivityNetworkSolver)(((SpatialFluentSolver2)this.metaCS.getConstraintSolvers()[0]).getConstraintSolvers()[1]);
			...
			for (String possibleOperatorTail : possibleOperator.getRequirementActivities()) {
				...
				if (possibleOperatorTailComponent.equals(possibleOperatorComponent) && possibleOperatorTailSymbol.equals(possibleOperatorSymbol)) {
					...
					VariablePrototype tailActivity = new VariablePrototype(
							(ActivityNetworkSolver)(((SpatialFluentSolver2)this.metaCS.getConstraintSolvers()[0]).getConstraintSolvers()[1]),
							possibleOperatorTailComponent, possibleOperatorTailSymbol);
					
					
//					Activity tailActivity = (Activity)((ActivityNetworkSolver)(((SpatialFluentSolver2)this.metaCS.getConstraintSolvers()[0]).
...
//					tailActivity.setSymbolicDomain(possibleOperatorTailSymbol);
	...
	public ConstraintNetwork[] getMetaValues(MetaVariable metaVariable) {
		...
		SpatialFluentSolver2 s = ((SpatialFluentSolver2)this.metaCS.getConstraintSolvers()[0]);
...
//			}
	...
	public HashMap<String, SimpleReusableResource3> getResources() {
		return resourcesMap;
	}
	// Given a variable act, it returns all the RubReusRes that are currently exploited by the variable
	public SimpleReusableResource3[] getCurrentReusableResourcesUsedByActivity(Variable act) {
		Vector<SimpleReusableResource3> ret = new Vector<SimpleReusableResource3>();
		for (SimpleReusableResource3 rr : currentResourceUtilizers.keySet()) {
			if (currentResourceUtilizers.get(rr).containsKey(act)) 
				ret.add(rr);
		}
		return ret.toArray(new SimpleReusableResource3[ret.size()]);
	}

	public int getResourceUsageLevel(SimpleReusableResource3 rr, Variable act) {
